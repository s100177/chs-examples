...
本脚本通过调用根目录的run_scenario模块来运行仿真场景，
...
def run_yinchuojiliao_scenario():
    """
    运行引绰济辽工程仿真场景
    """
    print("\n=== 引绰济辽工程仿真 (场景方式) ===")
    ...
    try:
        ...
        if missing_files:
            print(f"❌ 缺少必要的配置文件: {', '.join(missing_files)}")
            return False
        
        print(f"场景目录: {scenario_path}")
        print("配置文件检查通过")
...
def main():
    ...
    success = False
    if scenario_num == 1:
        success = run_yinchuojiliao_scenario()
    else:
